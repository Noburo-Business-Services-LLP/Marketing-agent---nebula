...
const contentCalendarSchema = new mongoose.Schema({
  ...
  // How many posts auto-generation may produce for this month. A week's worth
  // by default: left uncapped it worked through all 30 planned days, one per
  // day, with no way to ask for less.
  autoGenerateLimit: { type: Number, default: 7, min: 1, max: 31 },
  // Counts what auto-generation itself has made, so the cap is not confused by
  // posts the user created by hand from the same plan.
