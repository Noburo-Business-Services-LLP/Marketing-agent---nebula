...
const DEFAULT_CONCURRENCY = Math.max(
  1,
  Number.parseInt(process.env.CAMPAIGN_IMAGE_QUEUE_CONCURRENCY || '2', 10) || 2
);
const DEFAULT_JOB_TTL_MS = Math.max(
  10 * 60 * 1000,
  Number.parseInt(process.env.CAMPAIGN_IMAGE_JOB_TTL_MS || String(6 * 60 * 60 * 1000), 10) || (6 * 60 * 60 * 1000)
);

class InMemoryCampaignImageQueue {
  ...
  async _runJob(task) {
    ...
    try {
      ...
      this._updateJob(jobId, {
        ...
        completedAt: new Date().toISOString(),
        error: {
          message: error?.message || 'Campaign image generation failed',
...
const campaignImageQueue = new InMemoryCampaignImageQueue();

module.exports = {
  InMemoryCampaignImageQueue,
  campaignImageQueue
};
