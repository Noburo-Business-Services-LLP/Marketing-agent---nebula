...
- Character becomes optional unless explicitly enabled.`;
  ...
  const systemPrompt = await buildPrompt(user?._id, 'video.story', {
    name: (profile?.name || 'N/A'),
    industry: (profile?.industry || 'N/A'),
    targetAudience: (profile?.targetAudience || 'General audience'),
    brandToneFromProfile: (brandToneFromProfile),
    brandSummary: (brandSummary || 'N/A'),
    description: (input.description),
    durationSeconds: (input.durationSeconds),
    sceneCount: (sceneCount),
    wordTarget: (Math.round(input.durationSeconds * 2)),
    videoStyleContext: (videoStyleContext),
...
- Cast the visualDescription honestly: if you write "she walks through the room", her ID must be in charactersRequired. If you write "sunlight falls across the table", charactersRequired can be empty.`
  ...
  const prompt = await buildPrompt(user?._id, 'video.storyboard', {
    name: (profile?.name || 'N/A'),
    industry: (profile?.industry || 'N/A'),
    targetAudience: (profile?.targetAudience || 'General audience'),
    brandToneFromProfile: (brandToneFromProfile),
    brandSummary: (brandSummary || 'N/A'),
    description: (input.description),
    characterBibleBlock: (characterBibleBlock),
    durationSeconds: (input.durationSeconds),
    sceneCount: (sceneCount),
    block10: (durations.join(', ')),
    voiceLanguageDirective: (voiceLanguageDirective),
    environmentDirective: (environmentDirective),
    block13: (Math.round(input.durationSeconds * 2)),
    targetLangLabel: (targetLangLabel),
    targetScript: (targetScript),
    block16: (durations[0] || 3),
    block17: (durations[1] || 3),
    block18: (durations[2] || 3),
    block19: (characters && characters.length >= 2 ? JSON.stringify(characters.slice(0, 2).map((c) => c.id)) : '["01"]')
  });
...
async function generateSingleScene({ input, product, user, story, voiceScript, globalVisualStyle, scenesSoFar, currentSceneSkeleton, characters = [], castImageUrl = '', logger = null }) {
  ...
  const prompt = await buildPrompt(user?._id, 'video.scene', {
    name: (profile?.name || 'N/A'),
    industry: (profile?.industry || 'N/A'),
    brandToneFromProfile: (brandToneFromProfile),
    hook: (story?.hook || ''),
    beginning: (story?.beginning || ''),
    emotionalProgression: (story?.emotionalProgression || ''),
    climax: (story?.climax || ''),
    brandReveal: (story?.brandReveal || ''),
    ending: (story?.ending || ''),
    voiceScript: (voiceScript || '(none)'),
    globalVisualStyle: (globalVisualStyle || 'Cinematic vertical commercial'),
    characterBibleBlock: (characterBibleBlock),
    block13: (previousScenesSummary ? 'PREVIOUS SCENES (already elaborated — keep continuity):\n' + previousScenesSummary : 'This is the first scene.'),
    ...
    block18: (JSON.stringify(currentSceneSkeleton.charactersRequired || [])),
    environmentDirective: (environmentDirective),
    block20: ((currentSceneSkeleton.charactersRequired || []).length > 1
...
async function enrichImagePromptsWithLLM({ scenes, plan, input, product, profile, referenceImageSource, userId = null, logger }) {
...
- Consistency Strength: ${input.characterConsistencyStrength || 'Loose'}`
  ...
  const systemPrompt = await buildPrompt(userId, 'video.imagePrompt', {
    referenceClause: (referenceClause),
    name: (profile?.name || 'N/A'),
    industry: (profile?.industry || 'N/A'),
    targetAudience: (profile?.targetAudience || 'General audience'),
    brandToneFromProfile: (brandToneFromProfile),
    characterBlock: (characterBlock),
    globalVisualStyle: (plan?.globalVisualStyle || 'Premium cinematic vertical ad style with consistent lighting and color palette.'),
    block8: (scenes.map((s, i) => `Scene ${i + 1} — id: ${s.sceneId}
  Title: ${s.title || ''}
...
async function enrichVideoPromptsWithLLM({ scenes, plan, input, product, profile, userId = null, logger }) {
  if (!Array.isArray(scenes) || scenes.length === 0) return scenes;
  ...
  const systemPrompt = await buildPrompt(userId, 'video.motionPrompt', {
    characterMotionBlock: (characterMotionBlock),
    name: (profile?.name || 'N/A'),
    industry: (profile?.industry || 'N/A'),
    brandToneFromProfile: (brandToneFromProfile),
    globalVisualStyle: (plan?.globalVisualStyle || 'Premium cinematic vertical ad style.'),
    block6: (scenes.map((s, i) => `Scene ${i + 1} — id: ${s.sceneId}
  Title: ${s.title || ''}
...
async function enrichVoiceoverWithLLM({ voiceScript, sceneData, input, product, profile, languageCode, userId = null, logger }) {
  ...
  const systemPrompt = await buildPrompt(userId, 'video.voiceover', {
    name: (profile?.name || 'N/A'),
    industry: (profile?.industry || 'N/A'),
    name2: (product?.name || profile?.description || 'N/A'),
    targetAudience: (profile?.targetAudience || 'General audience'),
    brandToneFromProfile: (brandToneFromProfile),
    sceneSummary: (sceneSummary || String(source).slice(0, 400)),
    durationSeconds: (durationSeconds),
    targetWords: (targetWords),
    minWords: (minWords),
    maxWords: (maxWords),
    block11: (isTamil ? `LANGUAGE — TAMIL REQUESTED
...
`),
    block12: (isTamil ? '"tanglishVoiceScript": "string — romanized Tamil version of voiceScript",' : '')
  });
...
async function recommendMusicWithLLM({ voiceScript, sceneData, input, product, profile, userId = null, logger }) {
  const durationSeconds = Number(input?.durationSeconds || 60) || 60;
  const brandToneFromProfile = Array.isArray(profile?.brandVoice)
    ? profile.brandVoice.join(', ')
    : String(profile?.brandVoice || profile?.tone || 'Emotional');
  ...
  const systemPrompt = await buildPrompt(userId, 'video.music', {
    name: (profile?.name || 'N/A'),
    industry: (profile?.industry || 'N/A'),
    name2: (product?.name || 'N/A'),
    brandToneFromProfile: (brandToneFromProfile),
    durationSeconds: (durationSeconds),
    sceneSummary: (sceneSummary || 'N/A'),
    block7: (String(voiceScript || '').slice(0, 800)),
    block8: (Math.round(durationSeconds * 0.25)),
    block9: (Math.round(durationSeconds * 0.2))
  });
