...
const UploadPublishModal: React.FC<UploadPublishModalProps> = ({ onClose, onSuccess, isDarkMode, theme, connectedPlatforms }) => {
    ...
    const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            reader.onload = () => {
                ...
                setImageUrl(null); // Reset hosted URL
            };
            reader.readAsDataURL(file);
        }
    };
    ...
    const handleGenerateCaption = async () => {
        ...
        try {
            const response = await apiService.generateCaptionFromImage(uploadedImage, selectedPlatforms[0] || 'instagram');
    ...
    const handlePublish = async () => {
        ...
        try {
            // Create campaign with uploaded image
            const { campaign } = await apiService.createCampaign({
            ...
            if (publishRes.success) {
                ...
                setPublishResult({ success: false, message: publishRes.message || 'Failed to publish' });
            }
        } catch (error: any) {
            setPublishResult({ success: false, message: error.message || 'Failed to publish' });
        }
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={onClose}>
            <div 
                className={`rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden ${theme.bgCard}`}
                onClick={e => e.stopPropagation()}
            >
                {/* Header */}
                <div className={`flex items-center justify-between p-5 border-b ${isDarkMode ? 'border-slate-700/50' : 'border-slate-200'}`}>
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className={`text-xs ${theme.textSecondary}`}>Upload your poster and publish to social media</p>
                        </div>
                    </div>
                    <button onClick={onClose} className={`p-2 rounded-lg transition-colors ${isDarkMode ? 'hover:bg-slate-700' : 'hover:bg-slate-100'}`}>
                        <X className="w-5 h-5 text-slate-400" />
                        ...
                        {!uploadedImage ? (
                            <div 
                                className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-colors ${
                                    isDarkMode 
                                        ? 'border-slate-700 hover:border-[#ffcc29]/50 bg-slate-800/30' 
                                        : 'border-slate-300 hover:border-[#ffcc29]/50 bg-slate-50'
                                }`}
                            >
                                <input
                                    type="file"
                                    ...
                                    className="hidden"
                                    id="upload-publish-image"
                                />
                                <label htmlFor="upload-publish-image" className="cursor-pointer">
                                    <div className="flex flex-col items-center gap-3">
                                        <div className="p-4 bg-green-500/10 rounded-full">
                                            <ImageIcon className="w-8 h-8 text-green-500" />
                                        </div>
                                        <div>
                                            <p className={`font-medium ${theme.text}`}>Drop your image here</p>
                                            <p className={`text-sm ${theme.textSecondary}`}>or click to browse (PNG, JPG)</p>
                                        </div>
                                    </div>
                                </label>
                            ...
                            <div className="relative">
                                <img 
                                    src={uploadedImage} 
                                    alt="Uploaded" 
                                    className="w-full max-h-64 object-contain rounded-xl border border-slate-700/50"
                                />
                                <button 
                                    onClick={() => setUploadedImage(null)}
                                    className="absolute top-2 right-2 p-2 bg-red-500/80 rounded-full text-white hover:bg-red-600"
                                >
                                    <X className="w-4 h-4" />
                                </button>
                            </div>
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-2">
                            ...
                            <button
                                ...
                                disabled={!uploadedImage || isGeneratingCaption}
                                className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                                    uploadedImage && !isGeneratingCaption
                        ...
                        <textarea
                            value={caption}
                            onChange={e => setCaption(e.target.value)}
                            placeholder="Write your caption here or click 'Generate with AI' to auto-generate..."
                            rows={4}
                            className={`${inputClasses} resize-none`}
                        />
                    </div>
                ...
                <div className={`flex items-center justify-between p-5 border-t ${isDarkMode ? 'border-slate-700/50' : 'border-slate-200'}`}>
                    ...
                    <button 
                        onClick={handlePublish}
                        disabled={isPublishing || !uploadedImage || !caption.trim() || selectedPlatforms.length === 0}
