...
const Campaigns: React.FC = () => {
  ...
  const handlePublish = async () => {
    ...
    try {
      const result = await apiService.publishCampaign(postingCampaign._id, selectedPlatforms);
      
      if (result.success) {
        setPublishResult({ success: true, message: `Posted successfully to ${selectedPlatforms.join(', ')}!` });
        // Update campaign status in local state
        setCampaigns(prev => prev.map(c => 
          c._id === postingCampaign._id ? { ...c, status: 'posted' } : c
        ));
          ...
          <div className={`w-full max-w-md rounded-2xl shadow-2xl ${isDarkMode ? 'bg-[#161b22]' : 'bg-white'}`}>
            ...
            <div className="p-4">
              ...
              <div className="space-y-2">
                {['Instagram', 'Facebook', 'X', 'LinkedIn', 'YouTube', 'Pinterest'].map(platform => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        {platform === 'Pinterest' && <Share2 className="w-5 h-5" />}
